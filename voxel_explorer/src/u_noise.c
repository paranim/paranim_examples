...
// wraps as (x>>s) % 65536
// output is 0..65535
int big_noise(int x, int y, int s, unsigned int seed)
{
...
#else
#define rot(x,k)  (((x)<<(k)) + ((unsigned int)(x)>>(32-(k))))
#endif

// input is x,y; output is 0..2^32-1; doesn't repeat
unsigned int flat_noise32_weak(int rawx, int rawy, unsigned int seed)
{
   // burtle lookup3
   ...
   unsigned int c = (unsigned int) rawx;
   unsigned int b = (unsigned int) rawy;
...
// input is x,y; output is 0..2^32-1; doesn't repeat
unsigned int flat_noise32_strong(int rawx, int rawy, unsigned int seed)
{
   // burtle lookup3
   // original was 55 logic/arithmetic ops, critical path 26 instructions
   // modified with #if 0 gives 47 ops, critical path 18
...
static const unsigned int stb__sha256_K[64] = {
...
void stb_sha256_noise(unsigned int result[8], unsigned int x, unsigned int y, unsigned int seed, unsigned int param)
{
	unsigned int W[16];
